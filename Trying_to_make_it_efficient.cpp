...
int check_clause_status(vector<int> clause, map<int,int> variable_assignment){
    ...
    for(int literal : clause){
        auto varass = variable_assignment[abs(literal)];
        if(varass!= 0){
            k++;
            if(varass * literal < 0){
                continue;
...
int unit_propogation(vector<vector<int>>& clauses, map<int,int>& variable_assignment){
    while(true){
            ...
            for(int i=0;i<clauses.size();i++){
                ...
                // auto boombaam = check_clause_status_and_number_of_unassaigned_variables(clause, variable_assignment);
                auto ccs = check_clause_status(clause, variable_assignment);
                // auto numberofunassigned = boombaam.second;
                if(ccs==-1){
                        return -1;
                        // Conflict Mil gaya
                }
